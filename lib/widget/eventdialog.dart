  ...
  showModalBottomSheet(
    ...
    builder: (context) => ChangeNotifierProvider(
      create: (_) {
        ...
        form.init(event, expenseCategories.isNotEmpty ? expenseCategories.first : "General");
        
        nameController.text = form.name;
        amountController.text = form.totalAmount > 0 ? form.totalAmount.toString() : "";
        membersController.text = form.members > 0 ? form.members.toString() : "";
              ...
              children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: const InputDecoration(labelText: "Members", border: OutlineInputBorder()),
                      ),
                    ),
                  ],
                ),
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Text("Split Amount", style: TextStyle(color: Colors.white70)),
                      Text("₹ ${form.splitAmount.toStringAsFixed(2)}", 
                ...
                InkWell(
                  onTap: () async {
                    ...
                    if (result != null) form.updateAttachment(result.files.single.path);
                  },
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: Text(form.attachmentPath == null ? "Attach File" : form.attachmentPath!.split('/').last, 
                            style: const TextStyle(color: Colors.white70), overflow: TextOverflow.ellipsis)),
                        if (form.attachmentPath != null) const Icon(Icons.check_circle, color: Colors.greenAccent),
                      ],
                    ),
                  ),
                ),
                const SizedBox(height: 20),
                ElevatedButton(
                  style: ElevatedButton.styleFrom(minimumSize: const Size.fromHeight(55), backgroundColor: Colors.indigo),
                  onPressed: () {
                    if (form.name.isEmpty || form.totalAmount <= 0) return;

                    final resultEvent = event ?? EventModel(
                      name: form.name,
                  ...
                  child: Text(event == null ? "Add Event" : "Update Event", style: const TextStyle(color: Colors.white)),
                ),
              ],
            ),
          ),
